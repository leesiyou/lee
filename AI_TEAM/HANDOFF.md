...
| 字段 | 值 |
|---|---|
| TASK_ID | `AI-TEAM-HANDOFF-001`（本迁移任务）；内含演练 `HANDSHAKE-000`（链路）/ `HANDSHAKE-001`（QA 演练）/ `HANDSHAKE-002`（修复演练） |
| repo | `leesiyou/lee` |
| branch | `task/ai-team-handoff-001`（自 `1318969` 起的独立 worktree，未动共享分支） |
| BASE_HEAD | `1318969b5eddbec02b2357e734a034130769de38` |
| FINAL_HEAD | 由交付时 QA_REQUEST 信封的 `current_head` 记录（分支 tip 即 FINAL_HEAD；不写死在本文件以免自引用失真） |
| 日期 | 2026-09-21 |
